...
<p class="text-xs font-medium text-slate-500 dark:text-slate-400 mb-5 max-w-3xl leading-relaxed">
    Your personal finances — scoped to this login only.
    Quick capture stays in <a class="font-bold text-teal-700 dark:text-teal-400 hover:underline" href="<?= Str::e(Url::to('/app/add')) ?>">Mobile add</a>.
</p>

<?php if ($message): ?>
<?php View::partial('components/ui/message-banner-inline', ['tone' => 'success', 'icon' => 'check-circle', 'message' => (string) $message]); ?>
<?php endif; ?>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6 mb-4">
<!-- ─── Hero Net Worth Card ───────────────────────────────── -->
<div class="lg:col-span-2 rounded-2xl bg-gradient-to-br from-teal-600 via-teal-700 to-teal-900 text-white p-6 md:p-7 shadow-xl shadow-teal-500/20 relative overflow-hidden min-w-0">
...
<!-- ─── KPI 2×2 Grid ──────────────────────────────────────── -->
<div class="grid grid-cols-2 lg:grid-cols-2 gap-3 sm:gap-4 min-w-0">

    <div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-4 sm:p-5 shadow-sm min-h-[136px] sm:min-h-[148px] min-w-0">
        ...
        <p class="text-xl sm:text-2xl font-extrabold text-emerald-600 tabular-nums tracking-tight">RM <?= number_format($totalIncome, 0) ?></p>
    ...
    <div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-4 sm:p-5 shadow-sm min-h-[136px] sm:min-h-[148px] min-w-0">
        ...
        <p class="text-xl sm:text-2xl font-extrabold text-rose-600 tabular-nums tracking-tight">RM <?= number_format($totalExpense, 0) ?></p>
        <p class="text-[10px] text-slate-400 mt-0.5 font-medium">All time total</p>
    </div>
    ...
    <div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-4 sm:p-5 shadow-sm min-h-[136px] sm:min-h-[148px] min-w-0">
        ...
        <p class="text-xl sm:text-2xl font-extrabold text-teal-700 dark:text-teal-400 tabular-nums tracking-tight">RM <?= number_format($totalSavings, 0) ?></p>
    ...
    <div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-4 sm:p-5 shadow-sm min-h-[136px] sm:min-h-[148px] min-w-0">
        ...
        <p class="text-xl sm:text-2xl font-extrabold text-violet-700 dark:text-violet-400 tabular-nums tracking-tight"><?= $savingsRate ?>%</p>
...
<div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-5 md:p-6 lg:p-7 mb-4 md:mb-6 shadow-sm min-w-0">
    <div class="flex items-center justify-between mb-3">
        ...
        <a href="<?= Str::e(Url::to('/dashboard/transactions')) ?>" class="text-xs text-teal-700 dark:text-teal-400 font-semibold flex items-center gap-1 hover:underline">
            Transaction intelligence <i data-lucide="arrow-right" class="w-3 h-3"></i>
        </a>
    </div>
    <div id="dashCashflowChart" class="min-h-[200px] md:min-h-[260px] lg:min-h-[280px] w-full"></div>
</div>
<?php endif; ?>
...
<div class="mb-4">
    <div class="flex items-center justify-between mb-2.5">
        ...
        <a href="<?= Str::e(Url::to('/dashboard/wallets')) ?>" class="text-xs text-teal-700 dark:text-teal-400 font-semibold flex items-center gap-1 hover:underline">
            Analytics <i data-lucide="arrow-right" class="w-3 h-3"></i>
        </a>
    </div>
...
<div class="rounded-2xl bg-white dark:bg-[#0d1424] border border-slate-200/60 dark:border-slate-800 p-4 mb-4 shadow-sm">
    <div class="flex items-center justify-between mb-3">
        ...
        <a href="<?= Str::e(Url::to('/dashboard/recurring')) ?>" class="text-xs text-teal-700 dark:text-teal-400 font-semibold hover:underline">All →</a>
    </div>
...
(function () {
    ...
    var chartEl = document.getElementById('dashCashflowChart');
    if (chartEl && months.length) {
        new ApexCharts(chartEl, {
            ...
            xaxis: {
                categories: months,
                labels: {
                    style: { colors: text, fontSize: window.matchMedia('(min-width:1024px)').matches ? '11px' : '10px' },
                    maxHeight: 64,
                    trim: months.length > 8
                },
            ...
            legend: { show: true, position: window.matchMedia('(min-width:900px)').matches ? 'top' : 'bottom', fontSize: '11px', labels: { colors: text }, offsetY: window.matchMedia('(min-width:900px)').matches ? 0 : 8 },
            ...
            responsive: [
                {
                    ...
                    options: {
                        ...
                        chart: { height: 224 },
                        markers: { size: months.length <= 8 ? 3 : 0 }
                    }
                }
            ]
        }).render();
